...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (fetchError) return NextResponse.json({ error: 'Unable to look up this booking.' }, { status: 500 })
  ...
  if (booking.status !== 'cancelled') {
    ...
    if (cancelError) return NextResponse.json({ error: 'Unable to cancel the booking before deletion.' }, { status: 500 })
  }
  ...
  if (deleteError) return NextResponse.json({ error: 'Unable to delete the booking.' }, { status: 500 })
